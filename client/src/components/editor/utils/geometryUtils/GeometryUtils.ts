import { Box3, BufferGeometry, ClampToEdgeWrapping, Color, CylinderBufferGeometry, DoubleSide, Float32BufferAttribute, Group, LineBasicMaterial, LineSegments, Material, Matrix4, Mesh, MeshBasicMaterial, MeshStandardMaterial, Object3D, Quaternion, RepeatWrapping, SphereBufferGeometry, Vector2, Vector3, Wrapping } from "three";
...
export function createBoundaryCylinder(mesh: Mesh, mode: 'edge' | 'vertical_edge' | 'vertex') {
    ...
    if (mode === 'vertical_edge') {
        for (let boundary of boundaries) {
            ...
            const cylinder = new CylinderBufferGeometry(dist / 50, dist / 50, Math.abs(v1.y - v2.y + 0.1), 10, 1);
        ...
        for (let boundary of boundaries) {
            ...
            const cylinder = new CylinderBufferGeometry(dist / 50, dist / 50, Math.abs(_v1.distanceTo(_v2) + 0.1), 10, 1);
