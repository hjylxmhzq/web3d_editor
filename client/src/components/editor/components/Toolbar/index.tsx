...
import { getAllVersions, mergeTilesVersions, Versions } from '../../../../api/vertionControl';
import axios from 'axios';
import { getBaseVersion, getLastVersions, mergeTilesets } from '../../utils/versionControl/VersionControl';
...
export default function Toolbar({ }: IProps) {
  ...
  useEffect(() => {
    ...
    function get3DTilesURL(version: string) {
      return `http://127.0.0.1:8999/3dtiles_scene/${sceneSettings.action.selectScene}/tileset${version ? '_' + version : ''}.json`;
    }
    ...
    async function mergeVersions() {
      ...
      await mergeTilesVersions(sceneSettings.action.selectScene, left, right, versionTag, JSON.stringify(merged, null, 2));
    ...
    observe(() => sceneSettings.action.mergeVersions, async () => {
      await mergeVersions();
    });
